...
Problem:
  ...
  Rules:
    ...
    -look for the only duplicate value
...
Data Structures:
  ...
  Input  = array
  Output = number (value of the only duplicat3e value in the argument)
  
Algorithm:

  -Create a counter `currentValue` to return
  -Sort the argument
  -Iterate over the argument
    -on each iteration 
      -if current element is not equal to `currentValue`
        -reassign `currentValue` to element
      -else
        -return `currentVale`
